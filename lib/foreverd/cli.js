...
var ForeverService = require('./service');
...
function router(app) {
    app.use(function(cmd,tty,next) {
        ...
        var uid = options.uid || forever.randomString(24);
        options.uid = uid;
    ...
    app.cli('/stop', function(cmd,tty) {
        cmd.service.adapter.getClient(function (err, client) {
            if(!err) {
                client.emit('foreverd.cmd.stop');
            }
            client.endStreams();
        });
    });
    app.cli('/restart', function(cmd,tty) {
        cmd.service.adapter.getClient(function (err, client) {
            if(!err) {
                client.emit('foreverd.cmd.restart');
            }
            client.endStreams();
        });
    });
    app.cli('/list', function(cmd,tty) {
        cmd.service.adapter.getClient(function (err, client) {
            ...
            client.on('foreverd.listed', function(err, applications) {
                applications.forEach(function printApplication(application) {
                    console.error(application.file, application.options)
                });
                client.endStreams();
            });
        });
    ...
    app.cli('/pause', function(cmd,tty) {
        cmd.service.adapter.getClient(function (err, client) {
            if(!err) {
                client.emit('foreverd.cmd.pause');
            }
            client.endStreams();
        });
    });
    ...
    app.cli('/tail', function(cmd,tty) {
        cmd.service.adapter.getClient(function (err, client) {
            client.on('foreverd.*', function() {
                console.error(this.event, arguments);
            });
        });
    });
}
